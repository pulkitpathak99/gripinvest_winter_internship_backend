...
jest.mock('../controllers/product.controller', () => {
  return {
    ProductController: jest.fn().mockImplementation(() => {
      ...
      return {
        getAllProducts: (req: Request, res: Response) => res.status(200).json(mockProducts),
        createProduct: createProductLogic,
        // Add placeholder mocks for all other methods to prevent crashes
        getProductById: jest.fn((req: Request, res: Response) => res.status(200).json({})),
        getProductAnalysis: jest.fn((req: Request, res: Response) => res.status(200).json({})),
        updateProduct: jest.fn((req: Request, res: Response) => res.status(200).json({})),
        deleteProduct: jest.fn((req: Request, res: Response) => res.status(204).send()),
        generateDescription: jest.fn((req: Request, res: Response) => res.status(200).json({}))
      };
    }),
  };
});
