...
const investmentServiceMock = investmentService as jest.Mocked<typeof investmentService>;
const portfolioServiceMock = portfolioService as jest.Mocked<typeof portfolioService>;
...
describe('Investment and Portfolio Routes', () => {
  ...
  beforeEach(() => {
    ...
    prismaMock.user.findUnique.mockResolvedValue({ id: userId, role: 'USER' } as any);
  });
  ...
  describe('POST /investments', () => {
    ...
    test('✅ should return 201 and the new investment on success', async () => {
      ...
      investmentServiceMock.createInvestment.mockResolvedValue(mockInvestment as any);
      ...
      expect(investmentServiceMock.createInvestment).toHaveBeenCalledWith(userId, investmentData.productId, investmentData.amount);
    });

    test('❌ should return 400 if the investment service throws an error', async () => {
        // Arrange
        investmentServiceMock.createInvestment.mockRejectedValue(new Error('Product not found'));
  ...
  describe('GET /portfolio/details', () => {
    ...
    test('✅ should return 200 and portfolio data on success', async () => {
        ...
        portfolioServiceMock.getPortfolioDetails.mockResolvedValue(mockPortfolio as any);
        ...
        expect(portfolioServiceMock.getPortfolioDetails).toHaveBeenCalledWith(userId);
    });
  });
});