...
const generateDetailedPerformance = (
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    runningValue =
      (runningValue + monthlyContributions) * (1 + marketFluctuation);
    ...
    history.push({
      date: monthStartDate.toLocaleString("default", {
        month: "short",
        year: "2-digit",
      }),
...
export const getPortfolioDetails = async (userId: string) => {
  ...
  const investmentsWithCurrentValue = investments.map((inv) => {
    ...
    return {
      ...
      currentValue: parseFloat(
        (Number(inv.amount) * simulatedGainLossFactor).toFixed(2),
      ),
    };
  });
  ...
  const assetAllocation = Array.from(distributionMap.entries()).map(
