...
export const createInvestment = async (
  ...
  if (new Decimal(amount).lessThan(product.minInvestment)) {
    throw new Error(
      `Investment amount is less than the minimum of ${product.minInvestment}`,
    );
  }
  if (
    product.maxInvestment &&
    new Decimal(amount).greaterThan(product.maxInvestment)
  ) {
    throw new Error(
      `Investment amount exceeds the maximum of ${product.maxInvestment}`,
    );
  }
