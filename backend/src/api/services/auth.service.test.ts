...
import { signupUser, loginUser, forgotPassword, resetPassword } from './auth.service';
...
describe('Auth Service', () => {
  ...
  describe('signupUser', () => {
    ...
    test('✅ should create and return a new user successfully', async () => {
      ...
      prismaMock.user.create.mockResolvedValue({ id: 'user_1', ...userData, passwordHash: hashedPassword, riskAppetite: 'moderate', role: 'USER' } as any);
    ...
    test('❌ should throw an error if email is already in use', async () => {
      prismaMock.user.findUnique.mockResolvedValue({ id: 'user_1', email: userData.email } as any);
      await expect(signupUser(userData)).rejects.toThrow('Email already in use');
  ...
  describe('loginUser', () => {
    ...
    test('✅ should return user and token on successful login', async () => {
      ...
      expect(jwt.sign).toHaveBeenCalledWith({ userId: mockUser.id }, process.env.JWT_SECRET, { expiresIn: '1d' });
  ...
  describe('Password Reset', () => {
    test('✅ forgotPassword should update user with reset token', async () => {
        ...
        prismaMock.user.findUnique.mockResolvedValue({ id: 'user_1', email } as any);
    ...
    test('❌ resetPassword should throw error for an invalid token', async () => {
        ...
        await expect(resetPassword('invalidToken', 'newPass')).rejects.toThrow('Token is invalid or has expired.');
    });
  });
});