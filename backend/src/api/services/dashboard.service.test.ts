  ...
  describe('fetchDashboardSummary', () => {
    ...
    test('✅ should return full summary for a user with investments', async () => {
      ...
      const mockInvestments = [
        { amount: 6000, investedAt: new Date('2025-09-10'), product: { investmentType: 'bond', name: 'Bond A' } },
        { amount: 4000, investedAt: new Date('2025-09-12'), product: { investmentType: 'etf', name: 'ETF B' } },
      ];
      ...
      expect(result.assetDistribution).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ name: 'BOND', value: 60.00 }),
          expect.objectContaining({ name: 'ETF', value: 40.00 }),
        ])
      );
    ...
    test('✅ should return empty state for a user with no investments', async () => {
      ...
      expect(result.aiInsight).toBe("You haven't made any investments yet. Explore our products to get started!");
    ...
    test('✅ should return fallback AI insight when generation fails', async () => {
      // Arrange
      const mockInvestments = [{ amount: 1000, investedAt: new Date(), product: { investmentType: 'bond', name: 'Bond A' } }];
      prismaMock.investment.findMany.mockResolvedValue(mockInvestments as any);
      generateContentWithFallbackMock.mockRejectedValue(new Error('AI Service Down'));
      ...
      // Assert
      expect(result.aiInsight).toBe("Your portfolio is looking good! Keep an eye on market trends for new opportunities.");
      expect(generateContentWithFallbackMock).toHaveBeenCalledTimes(1);
