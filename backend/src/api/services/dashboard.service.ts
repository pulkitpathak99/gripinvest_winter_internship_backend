...
async function getAiInsight(distribution: any[]): Promise<string> {
  ...
  const portfolioSummary = distribution
    .map((d) => `${d.value}% in ${d.name}`)
    .join(", ");
  const prompt = `
...
export const fetchDashboardSummary = async (userId: string) => {
  ...
  const assetDistribution = Array.from(distributionMap.entries()).map(
    ([name, value]) => ({
      name: name.toUpperCase(),
      value: parseFloat(((value / totalValue) * 100).toFixed(2)),
      color:
        name === "bond" ? "#3b82f6" : name === "etf" ? "#14b8a6" : "#f97316",
    }),
  );
