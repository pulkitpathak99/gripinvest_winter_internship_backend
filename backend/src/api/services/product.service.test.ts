...
describe("ProductService", () => {
  ...
  describe("Simple CRUD methods", () => {
    ...
    test("✅ getProductById should call prisma.findUnique", async () => {
      ...
      expect(prismaMock.investmentProduct.findUnique).toHaveBeenCalledWith({
        where: { id: productId },
      });
    });
    ...
    test("✅ updateProduct should call prisma.update", async () => {
      ...
      expect(prismaMock.investmentProduct.update).toHaveBeenCalledWith({
        where: { id: productId },
        data: updateData,
      });
    });
    ...
    test("✅ deleteProduct should call prisma.delete", async () => {
      ...
      expect(prismaMock.investmentProduct.delete).toHaveBeenCalledWith({
        where: { id: productId },
      });
    });
  });
  ...
  describe("AI Methods", () => {
    ...
    test("❌ getAIAnalysis should throw an error if product not found", async () => {
      ...
      await expect(productService.getAIAnalysis(productId)).rejects.toThrow(
        "Product not found",
      );
    });
