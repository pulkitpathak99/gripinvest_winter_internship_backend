...
describe('Investment Service', () => {
  ...
  describe('createInvestment', () => {
    ...
    test('❌ should throw an error if product is not found', async () => {
      ...
      await expect(createInvestment(userId, 'prod_invalid', amount)).rejects.toThrow('Product not found');
    });
    ...
    test('❌ should throw an error if amount is below minimum', async () => {
      ...
      // Act & Assert
      await expect(createInvestment(userId, mockProduct.id, lowAmount)).rejects.toThrow(`Investment amount is less than the minimum of ${mockProduct.minInvestment}`);
    });
    ...
    test('❌ should throw an error if amount is above maximum', async () => {
        ...
        // Act & Assert
        await expect(createInvestment(userId, mockProduct.id, highAmount)).rejects.toThrow(`Investment amount exceeds the maximum of ${mockProduct.maxInvestment}`);
    });
  });
  ...
  describe('cancelInvestment', () => {
    ...
    test('❌ should throw an error if investment is not found', async () => {
      ...
      // Act & Assert
      await expect(cancelInvestment(investmentId, userId)).rejects.toThrow('Investment not found or you do not have permission to cancel it.');
    });
    ...
    test('❌ should throw an error if investment is not active', async () => {
      ...
      // Act & Assert
      await expect(cancelInvestment(investmentId, userId)).rejects.toThrow('Only active investments can be cancelled.');
    });
    ...
    test('❌ should throw an error if cancellation period has expired', async () => {
      ...
      // Act & Assert
      await expect(cancelInvestment(investmentId, userId)).rejects.toThrow('Cancellation period has expired (24 hours).');
    });
  });
});