...
export default function PortfolioPage() {
  ...
  if (loading)
    return (
      <div className="text-center text-gray-300">Loading Your Portfolio...</div>
    );
  if (!portfolio)
    return (
  ...
  const beginningBalance =
    timeData[0]?.value -
    (timeData[0]?.value - timeData[0]?.contributions - timeData[0]?.earnings);
  const endingBalance = timeData[timeData.length - 1]?.value;
  const netContributions =
    timeData[timeData.length - 1]?.contributions - timeData[0]?.contributions;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <KpiCard
          ...
          value={`${formatCurrency(beginningBalance)}`}
          icon={Wallet}
        />
        <KpiCard
