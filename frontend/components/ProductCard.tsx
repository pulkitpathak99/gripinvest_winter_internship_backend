...
export default function ProductCard({ product, onInvestClick }: ProductCardProps) {
  return (
    <div className="bg-slate-800/50 border border-slate-800 rounded-xl p-6 flex flex-col justify-between transition-all hover:border-slate-700 hover:shadow-2xl hover:-translate-y-1">
      <div>
        ...
        <div className="flex justify-between items-start">
          <h2 className="text-xl font-semibold text-white pr-4">{product.name}</h2>
          <span className={`px-3 py-1 text-xs font-medium rounded-full border ${riskStyles[product.riskLevel]}`}>
            {product.riskLevel.charAt(0).toUpperCase() + product.riskLevel.slice(1)} Risk
          </span>
        </div>
        <p className="text-sm text-blue-400 mt-1">{product.investmentType.toUpperCase()}</p>
        ...
        <div className="space-y-3 border-t border-slate-800 pt-4 text-sm">
          <div className="flex justify-between items-center text-gray-300">
            <span className="flex items-center gap-2 text-gray-400"><Calendar size={16} /> Tenure</span>
            <span className="font-medium text-white">{product.tenureMonths} Months</span>
          </div>
          <div className="flex justify-between items-center text-gray-300">
