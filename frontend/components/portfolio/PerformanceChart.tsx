  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-800 border border-slate-700 p-3 rounded-lg shadow-lg text-sm">
        ...
        {payload[0] && (
          <p className="intro text-blue-400 font-semibold">{`Portfolio Value : ${formatCurrency(payload[0].value)}`}</p>
        )}
        {payload[1] && (
          <p className="intro text-teal-400">{`Net Contributions : ${formatCurrency(payload[1].value)}`}</p>
        )}
        {dataPoint.earnings !== undefined && (
          <p className="intro text-gray-300">{`Earnings : ${formatCurrency(dataPoint.earnings)}`}</p>
        )}
      </div>
    );
  }
