        ...
        <table className="w-full text-left">
          <thead className="border-b border-slate-700">
            <tr>
              ...
              <th className="p-4 text-sm font-medium text-gray-400">Endpoint</th>
          ...
          <tbody>
            {logs.map(log => (
              <tr key={log.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                <td className="p-4">
                  <span className={clsx('px-2.5 py-1 text-xs font-semibold rounded-full', getStatusClass(log.statusCode))}>
                    {log.statusCode}
                  </span>
                </td>
