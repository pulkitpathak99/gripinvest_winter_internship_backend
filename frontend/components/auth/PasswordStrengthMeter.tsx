...
const PasswordStrengthMeter = ({ password }: { password: string }) => {
  ...
  const getAiFeedback = () => {
    if (!password)
      return { text: "Create a strong password.", color: "text-gray-400" };
    if (score < 2)
      return {
        text: "Weak. Try adding more character types.",
        color: "text-red-400",
      };
    if (score < 4)
      return {
        text: "Good. Add a special character for extra strength.",
        color: "text-yellow-400",
      };
    return {
      text: "Excellent! This is a strong password.",
