...
const riskOptions = [
  { id: 'low', label: 'Low', description: 'Prioritize capital preservation with lower, more stable returns.', icon: ShieldCheck },
  { id: 'moderate', label: 'Moderate', description: 'A balanced approach seeking steady growth with manageable risk.', icon: ShieldQuestion },
  { id: 'high', label: 'High', description: 'Aim for maximum growth, accepting higher volatility.', icon: ShieldAlert },
];
        ...
        {riskOptions.map(option => (
          ...
          >
            <div className="flex items-center gap-3">
              <option.icon size={20} className={clsx(selectedRisk === option.id ? 'text-blue-400' : 'text-gray-400')} />
              <p className="font-semibold text-white">{option.label}</p>
            </div>
            <p className="text-xs text-gray-400 mt-1 pl-8">{option.description}</p>
          </button>
        ))}
      </div>
      <button 
