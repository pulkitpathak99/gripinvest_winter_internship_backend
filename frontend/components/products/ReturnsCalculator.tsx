...
export default function ReturnsCalculator({ product }: ReturnsCalculatorProps) {
  ...
  const interestEarned = investmentAmount * (product.annualYield / 100) * (product.tenureMonths / 12);
  ...
  return (
    <div className="bg-slate-800/50 border border-slate-800 rounded-xl p-6 mt-6">
      <h3 className="text-lg font-semibold text-white mb-4">Returns Calculator</h3>
      <div className="space-y-4">
        <div>
          <label className="block text-sm text-gray-400">Investment Amount</label>
          <input
            type="range"
          ...
          <div className="flex justify-between text-xs text-gray-500 mt-1">
            ...
            <span>{formatCurrency((product.maxInvestment || product.minInvestment * 10))}</span>
          </div>
        </div>
        ...
        <p className="text-xs text-gray-500 text-center pt-2">
          *This is a projection for fixed-return assets. Returns for MFs and ETFs are not guaranteed.
        </p>
      </div>
    </div>
  );
